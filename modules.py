...
def csr2arr(a: csr_matrix, top, training):
    ind = np.argsort(a.data)[::-1]
    col = np.append(a.indices[ind], [0] * (top - len(a.indices))).astype('int64')
    value = np.append(a.data[ind],  [0] * (top - len(a.data))).astype('float32')
    return col, value
...
class XMLDataset(Dataset):
    def __init__(self, features: csr_matrix, scores: csr_matrix, labels: csr_matrix = None, training=True):
        self.features, self.scores, self.labels, self.training = features, scores, labels, training
        self.ft_top = max(features.indptr[1:] - features.indptr[:-1])
        self.sc_top = max(scores.indptr[1:] - scores.indptr[:-1])
        self.lbl_top = max(labels.indptr[1:] - labels.indptr[:-1])
        #print(self.ft_top, self.sc_top, self.lbl_top)
    
    def __getitem__(self, index):
        ft_col, ft_value = csr2arr(self.features[index], self.ft_top, self.training)
        sc_col, sc_value = csr2arr(self.scores[index], self.sc_top, self.training)
        if self.training:
            lbl_col, lbl_value = csr2arr(self.labels[index], self.lbl_top, self.training)
...
class NormedLinear(nn.Module):
    ...
    def forward(self, x):
        out = x.mm(F.normalize(self.weight, dim=0)) + F.normalize(self.bias, dim=0)
        return out
...
class MLLinear(nn.Module):
    ...
    def get_linear(self, shape):
        if self.use_norm:
            fc_list = nn.ModuleList( NormedLinear(in_s, out_s) for in_s, out_s in zip(shape[0:-2], shape[1:-1]) )
